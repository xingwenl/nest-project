...
export class ArticleService {
  ...
  async addType(addType: AddTypeDto) {
    ...
    if (!isEmptyObj(type)) {
      httpRes(ApiErrorCode.DUPLICATE_VALUES, '类型不能重复', null);
    }
  ...
  getSocket(id: string): any {
    ...
    // }
    // this.eventsGateway.server.sockets[id]
  ...
  async findAllPage(type_id = 0, paging: any) {
    const testc = await this.articleRep.createQueryBuilder('a');
    let res = paging(testc);
    return res;
  }
  ...
  async home() {
    ...
    if (allType && allType.length) {
      let allArtice = await Promise.all(
        allType.map(obj => this.findAll(0, 5, obj.id)),
      );
