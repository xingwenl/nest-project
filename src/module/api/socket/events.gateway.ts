...
export class EventsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  onMessage(client: UserSocket, data: any): Observable<WsResponse<any>> | any {
    ...
    if (!client.login) {
      return returnData('message', errorData('请先登录'));
    }

    client.to(data.room).emit('message', successData(data));
    // 给已经登录的用户 发送消息
    // this.loginUsers.forEach((val: any) => {
    //   console.log(val.id)
    //   // console.log(this.server.sockets)
    //   this.server.sockets[val.id].emit('message', successData(data))
    // })
    // this.server.sockets.sockets[]
    // return returnData('message', successData(data))
  }
